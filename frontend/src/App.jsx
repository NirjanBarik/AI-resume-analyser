...
function ReportView({ result }) {
  ...
  return (
    <div className="animate-fade-in">
      <div className="card history-item flex items-center gap-8 mb-6">
...
export default function App() {
  const [view, setView] = useState(() => localStorage.getItem("token") ? "app" : "landing"); // landing | login | register | app | admin
  ...
  // ── RENDER: Landing & Auth Modal ────────────────────────────────────────────
  if (view === "landing" || view === "login" || view === "register") {
    const isAuth = view === "login" || view === "register";
    const isReg = view === "register";
    return (
      <div style={{ position: 'relative' }}>
        <LandingPage setView={setView} theme={theme} toggleTheme={toggleTheme} />
        
        {isAuth && (
          <div className="modal-backdrop" style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(4px)', animation: 'fadeIn 0.3s ease' }}>
            <div style={{ width: "100%", maxWidth: "400px", padding: "0 1.5rem", position: 'relative' }}>
              <div className="card animate-fade-in" style={{ padding: '2.5rem', position: 'relative', boxShadow: 'var(--shadow-lg)' }}>
                <button onClick={() => { setView("landing"); setAuthError(""); }} style={{ position: 'absolute', top: '1rem', right: '1rem', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-secondary)' }}>
                ...
                <div className="mb-6" style={{ textAlign: "center" }}>
                   ...
                   <p className="text-secondary text-sm mt-1">AI-powered resume analysis</p>
                </div>
                ...
                {authError && <div className="mb-4 text-sm font-medium" style={{ color: "var(--danger)" }}>{authError}</div>}
                
                <button className="btn btn-primary" style={{ width: "100%", padding: '0.8rem 1.4rem' }}
                ...
                <div className="text-center mt-6 text-sm text-secondary">
                  ...
                  <button style={{ background: "none", border: "none", color: "var(--text)", cursor: "pointer", fontWeight: 600, padding: 0 }}
        ...
        {tab === "history" && (
          <div>
            ...
            <div className="flex flex-col gap-4">
              {history.map((r) => (
                <div key={r._id} className="card history-item flex items-center gap-6">
                  ...
                  <div>
                    <button className="btn btn-secondary" onClick={() => setSelectedHistory(r)}>View Full Report</button>
                  </div>
                </div>
              ))}
            </div>
            ...
            {selectedHistory && (
              <div className="modal-backdrop" style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.6)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(6px)', animation: 'fadeIn 0.3s ease', padding: '1.5rem' }}>
                <div style={{ width: "100%", maxWidth: "960px", maxHeight: '90vh', overflowY: 'auto', position: 'relative', background: 'var(--bg-color)', borderRadius: 'var(--r-lg)', padding: '2rem' }}>
                  <div className="flex justify-between items-center mb-6">
                    <h2 className="text-2xl font-bold">Detailed Report</h2>
                    <button onClick={() => setSelectedHistory(null)} style={{ background: 'var(--surface2)', border: 'none', cursor: 'pointer', color: 'var(--text-secondary)', padding: '0.5rem', borderRadius: 'var(--r-sm)' }}>
                      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
                    </button>
                  </div>
                  <ReportView result={selectedHistory} />
                </div>
              </div>
            )}
          </div>
